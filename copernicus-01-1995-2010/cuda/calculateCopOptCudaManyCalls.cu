...
#pragma omp for schedule(guided)
	...
	*(startp+1)= k;//Siempre empezamos en la primera LON de la lat
	...
	if(!invalidValue){
	  ...
	  cudaMemcpy(device_part_tg_val, part_tg_val, TIME * sizeof(short),
		     cudaMemcpyHostToDevice);
	  thrust::sort(thrust::device, device_part_tg_val, device_part_tg_val + TIME);
