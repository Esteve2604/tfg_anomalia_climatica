...
int main(){
  ...
  total_pos = LAT*LON;
  printf("**Inicio** \n");
  //Reserva para escribir en el nuevo fichero lat y lon
  ...
  for(i=0; i<NUM_THREADS; i++){
    ...
    cudaMalloc((void**) &device_part_tg_val[i], TIME * sizeof(short));
  }
  ...
  free(lon);
  free(lat);
#pragma omp parallel private(retval,i,j,k,startp, countp, part_tg_val, estres_hidrico, estres_termico,anomalia_climatica, part_prec_val, device_drought_code, drought_code,last_drought_code,despl_lat,despl_total,invalidValue,timeId, orderedDroughtCode,orderedTg, precId,tgId, threadId, err) shared(ncIdPrec,ncIdTg,time, stream,device_part_tg_val, newAnomaliaClimaticaId, newNcId,notCalculated)  num_threads(NUM_THREADS)
  {
    threadId = omp_get_thread_num();
    
    // Construct a new `nosync` execution policy with the custom stream
    auto nosync_exec_policy = thrust::cuda::par_nosync.on(stream[threadId]);
    ...
    if((orderedDroughtCode = (float *)malloc(TIME*sizeof(float)))==NULL)
    ...
    cudaMalloc((void**) &device_part_tg_val, TIME * sizeof(short));
    ...
    *(countp+1)=1; //tantos datos como LON
...
#pragma omp for schedule(guided)
	...
	*startp = j;
	*(startp+1)= k;//Siempre empezamos en la primera LON de la lat
	...
	if(!invalidValue){
	  //Mandamos la temperatura para que la ordene la gpu
	  cudaMemcpy(device_part_tg_val[threadId], part_tg_val, TIME * sizeof(short),
		     cudaMemcpyHostToDevice);
	  thrust::sort(nosync_exec_policy, device_part_tg_val[threadId], device_part_tg_val[threadId] + TIME);
	  memcpy(orderedDroughtCode, drought_code, TIME);
	  //Ordenamos los datos para calcular los percentiles
	  thrust::sort(thrust::host, orderedDroughtCode, orderedDroughtCode + TIME);
	  //calculamos estres hidrico
	  calculateEstresHidrico(drought_code,orderedDroughtCode, estres_hidrico);
	  //Traemos la temperatura ordenada
	  cudaMemcpy(orderedTg, device_part_tg_val[threadId], TIME * sizeof(short),
		     cudaMemcpyDeviceToHost);
	
	  //calculamos el estrés térmico
...
#pragma omp critical
    ...
    cudaStreamDestroy(stream[threadId]);
    free(orderedTg);
    ...
    cudaFree(device_part_tg_val[threadId]);
