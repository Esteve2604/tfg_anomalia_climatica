...
int main(){
  ...
  short *d_keys; //Para back to back
  ...
  free(lon);
  free(lat);
#pragma omp parallel private(retval,i,j,k,startp, countp, part_tg_val, estres_hidrico, estres_termico,anomalia_climatica, part_prec_val, device_drought_code, drought_code,last_drought_code,despl_lat,despl_total,timeId, orderedDroughtCode,orderedTg, precId,tgId, threadId, writeStartp, writeCountp, invalidValue, device_part_tg_val, d_keys) shared(ncIdPrec,ncIdTg,time, newAnomaliaClimaticaId, newNcId,notCalculated)  num_threads(NUM_THREADS)
  {
    ...
    if((cudaMalloc((void**) &device_part_tg_val, TIME * sizeof(short)*LON)))
      MEMERR("Reserva device part tg val");
    if((cudaMalloc((void**) &d_keys, TIME * sizeof(short)*LON)))
      MEMERR("Reserva device part tg val");
...
#pragma omp for schedule(guided)
	...
	invalidValue[k] = calculateDroughtCode(part_tg_val+k*TIME, part_prec_val+k*TIME, drought_code+k*TIME, time);
      ...
      thrust::stable_sort_by_key(thrust::device, device_part_tg_val,
      ...
      thrust::stable_sort_by_key(thrust::device, d_keys,
				 d_keys+LON*TIME,
				 device_part_tg_val,
				 thrust::less<short>());
	...
	if(!invalidValue[i]){
      ...
      cudaMemcpy(orderedTg, device_part_tg_val, TIME * sizeof(short) * LON,
