...
int check_tg(short *tg, double *oldLon, double *oldLat, int lonSize, int latSize, int timeSize, short *newTg){
  ...
  for(i=0; i<latSize; i++){
	...
	if(*(tg+despl_total) < -8500 || *(tg+despl_total) > 8500){ //En caso de que sea nulo, copiamos el valor de un lugar cercano
	  if(*(tg+despl_total-1) > -8500 && *(tg+despl_total-1) < 8500) //A la izquierda
	    *(tg+despl_total) = *(tg+despl_total-1);
	  else if(*(tg+despl_total+1) > -8500 && *(tg+despl_total+1) < 8500) //Derecha
	    *(tg+despl_total) = *(tg+despl_total+1);
	  else if(*(tg+despl_total+lonSize) > -8500 && *(tg+despl_total+lonSize) < 8500) //Abajo
	    *(tg+despl_total) = *(tg+despl_total+lonSize);
	  else if(*(tg+despl_total-lonSize) > -8500 && *(tg+despl_total-lonSize) < 8500) //Arriba
	    *(tg+despl_total) = *(tg+despl_total-lonSize);
	}
