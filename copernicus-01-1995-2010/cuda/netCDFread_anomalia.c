...
#define FILE_NAME_PREC "./copernicus_anomalia_climatica_1995_2010.nc"
#define NDIMS 3
#define LAT 437
#define LON 592
#define TIME 5844
...
int data_analisis(short *val, double lat[], double lon[], double time[]){
  ...
  for(i = 0; i<LAT;i++){
	...
	if(*(val+ despl_total) < 1 || *((short *)val+ despl_total) > (short) 99){
	  not_valid++;
	  continue;
	}
	average_anom+=*(val+despl_total);
  ...
  printf("Total Data: %f Not Valid %ld, Percentage NV: %f%% Average_anom: %ld \n", total_data, not_valid,(not_valid/total_data)*100 , average_anom); 
  return 0;
}
